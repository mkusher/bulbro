...
describe("KeepingDistanceBehaviors", () => {
	const createTestEnemy = (position = zeroPoint()): EnemyState => {
		return spawnEnemy("enemy-1", position, aphidEnemy);
	};
	...
	describe("move", () => {
		it("should not move during starting raging phase", () => {
			const behavior = new KeepkingDistanceBehaviors();
		...
		it("should move away when player is too close (inside 0.75 range)", () => {
			const behavior = new KeepkingDistanceBehaviors();
			const now = nowTime(Date.now());
			const actualRange = aphidEnemy.stats.range / 4;
			const tooCloseDistance = actualRange * 0.5;
			const enemy = createTestEnemy({ x: 100, y: 100 });
			const player = createTestBulbro({ x: 100 + tooCloseDistance, y: 100 });
			const waveState = createWaveState([player]);
			...
			if (moveEvent && moveEvent.type === "enemyMoved") {
				const moveDirection = {
					x: moveEvent.to.x - moveEvent.from.x,
					y: moveEvent.to.y - moveEvent.from.y,
				};
				...
				const dotProduct =
					moveDirection.x * toPlayerDirection.x +
					moveDirection.y * toPlayerDirection.y;
		...
		it("should not move when player is in optimal range (0.75-1.0 range)", () => {
			...
			const now = nowTime(Date.now());
			const actualRange = aphidEnemy.stats.range / 4;
			const optimalDistance = actualRange * 0.85;
			const enemy = createTestEnemy({ x: 100, y: 100 });
			const player = createTestBulbro({ x: 100 + optimalDistance, y: 100 });
			const waveState = createWaveState([player]);
		...
		it("should move toward player when too far (beyond 1.0 range)", () => {
			...
			const now = nowTime(Date.now());
			const actualRange = aphidEnemy.stats.range / 4;
			const farDistance = actualRange * 1.5;
			const enemy = createTestEnemy({ x: 100, y: 100 });
			const player = createTestBulbro({ x: 100 + farDistance, y: 100 });
			const waveState = createWaveState([player]);
	...
	describe("attack", () => {
		it("should start raging when player is in range", () => {
			const behavior = new KeepkingDistanceBehaviors();
			const now = nowTime(Date.now());
			const actualRange = aphidEnemy.stats.range / 8;
			const enemy = createTestEnemy({ x: 100, y: 100 });
		...
		it("should not attack during starting raging phase", () => {
			...
			const player = createTestBulbro({ x: 200, y: 100 });
			const waveState = createWaveState([player]);
		...
		it("should shoot in remembered direction during second raging phase", () => {
			...
			if (shotEvent && shotEvent.type === "shot") {
				const shotDirection = {
					x: shotEvent.shot.direction.x,
					y: shotEvent.shot.direction.y,
				};
				expect(Math.abs(shotDirection.x)).toBeGreaterThan(
					Math.abs(shotDirection.y),
				);
			}
		});
