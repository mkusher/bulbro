...
export class InWaveStats {
	...
	constructor(canvasSize: Size, playerIndex: number) {
		...
		const width = this.#width(canvasSize);

		const leftBorder =
			this.#playerIndex === 0
				? this.#padding
				: canvasSize.width - width - this.#padding;

		this.#gfx.x = leftBorder;
		this.#gfx.y = this.#padding;

		this.#healthIndicator = new HealthSprite(width);
		this.#experienceIndicator = new ExperienceSprite(width);
		this.#materialsIndicator = new MaterialsIndicator(width, playerIndex);
		this.#healthIndicator.appendTo(this.#gfx);
	...
	update(canvasSize: Size, player?: BulbroState) {
		const width = this.#width(canvasSize);
		this.#healthIndicator.update(width, player);
		this.#experienceIndicator.update(width, player);
