...
export class BulbroState implements BulbroStateProperties {
	...
	attack(enemies: EnemyState[], deltaTime: DeltaTime, now: NowTime) {
		const baseEvents: BulbroEvent[] = [];
		if (!this.isAlive()) return baseEvents;
		this.weapons.forEach((weapon) => {
			const reloadTime = weapon.statsBonus.attackSpeed ?? 0;
			const attackSpeed = this.stats.attackSpeed;
			if (
				...
				const target = findClosestEnemyInRange(
					this,
					weapon,
					enemies.filter((e) => !e.killedAt),
				);
				if (target && isInRange(this, target, weapon)) {
					const shot = shoot(this, "player", weapon, target.position);

					// Generate attack event using player's hit method
					const attackEvent = this.hit(weapon.id, target.id, shot);
	...
	aim(enemies: EnemyState[]) {
		return new BulbroState({
			...this.#props,
			weapons: this.weapons.map((weapon, index) => {
				const target = findClosestEnemyInRange(
					this,
					{
						...weapon,
						statsBonus: {
							...weapon.statsBonus,
							range: Math.max(100, weapon.statsBonus?.range ?? 0) * 1.5,
						},
					},
					enemies.filter((e) => !e.killedAt),
				);
				...
				const offset = calculateWeaponPosition(
					weapon,
					index,
					this.weapons.length,
				);
				const directionMultiplier = this.lastDirection.x < 0 ? -1 : 1;
				const position = {
					x: this.position.x + offset.x * directionMultiplier,
					y: this.position.y + offset.y - BULBRO_SIZE.height / 2,
				};
				const aimingDirection = direction(position, target.position);
				const reversed = {
					x: aimingDirection.x * directionMultiplier,
					y: -aimingDirection.y,
				};
				return {
					...weapon,
					aimingDirection: reversed,
				};
			}),
		});
	}
	...
	beHit(
		damage: number,
		now: NowTime,
	): BulbroReceivedHitEvent | BulbroDiedEvent {
	...
	healByHpRegeneration(now: NowTime) {
	...
	applyEvent(event: GameEvent): BulbroState {
		switch (event.type) {
			case "bulbroInitializedForWave":
				...
				return new BulbroState({
					...this.#props,
					position: round(event.position),
					lastDirection: event.direction,
					lastMovedAt: event.occurredAt,
					lastHitAt: event.occurredAt,
					healedByHpRegenerationAt: event.occurredAt,
					healthPoints: this.stats.maxHp,
					killedAt: undefined,
					weapons: this.weapons.map((ws) => ({
						...ws,
						lastStrikedAt: event.occurredAt,
					})),
				});
...
export function spawnBulbro(
	...
	return new BulbroState({
		id,
		type: character.style.faceType,
		level,
		totalExperience: experience,
		position,
		speed: calculateStats(character.statBonuses).speed,
		healthPoints: calculateStats(character.statBonuses).maxHp,
		stats: calculateStats(character.statBonuses),
		weapons,
		lastMovedAt: 0,
		lastHitAt: 0,
