...
function createMockGameEventQueue(): GameEventQueue {
	...
	return {
		addEvent: mock((event: GameEvent) => events.push(event)),
		flush: mock(() => {
			const result = [...events];
			events.length = 0;
			return result;
...
function createMockInGameCommunicationChannel(): InGameCommunicationChannel {
	...
	return {
		...
		onMessage: mock((handler: (message: WebsocketMessage) => void) => {
			messageHandlers.push(handler);
		}),
		// Helper method to simulate receiving messages
		_simulateMessage: (message: WebsocketMessage) => {
...
function createMockRemoteControl(): RemoteRepeatLastKnownDirectionControl {
	return {
		onMessage: mock(),
		direction: { x: 0, y: 0 }, // zero direction vector (no movement)
	} as any;
}
...
function createTestBulbro(id: string, x = 0, y = 0): BulbroState {
	return new BulbroState({
		...
		lastDirection: { x: 1, y: 0 }, // right direction vector
	});
}
...
describe("StateSync", () => {
	...
	let mockInGameCommunicationChannel: InGameCommunicationChannel & {
		_simulateMessage: (msg: WebsocketMessage) => void;
	};
	...
	beforeEach(() => {
		...
		mockGameEventQueue = createMockGameEventQueue();
		mockInGameCommunicationChannel =
			createMockInGameCommunicationChannel() as any;
		mockRemoteControl = createMockRemoteControl();
		mockPlayerControl = createMockPlayerControl();
	...
	describe("Host StateSync", () => {
		beforeEach(() => {
			stateSync = new StateSync({
				...
				isHost: true,
				inGameCommunicationChannel: mockInGameCommunicationChannel,
				stateUpdater,
				gameEventQueue: mockGameEventQueue,
				currentState,
				localPlayerControl: mockPlayerControl,
				remoteControl: mockRemoteControl,
				waveProcess: mockWaveProcess,
			});
		});
		...
		it("should handle incoming guest state update and trigger ping pong", () => {
				...
				events: [
					{
						...
						from: { x: 200, y: 200 },
						to: { x: 250, y: 200 },
						direction: direction({ x: 200, y: 200 }, { x: 250, y: 200 }),
					},
				],
			};
			...
			mockInGameCommunicationChannel._simulateMessage(guestMessage);

			// Should trigger wave process tick
			expect(mockWaveProcess.tick).toHaveBeenCalled();

			// Should send response message with incremented version
			expect(mockInGameCommunicationChannel.send).toHaveBeenCalledWith(
		...
		it("should handle messages with old versions by sending ping-pong", () => {
			...
			const initialCallCount = (mockInGameCommunicationChannel.send as any).mock
				.calls.length;
			...
			mockInGameCommunicationChannel._simulateMessage(firstMessage);
			const callsAfterFirst = (mockInGameCommunicationChannel.send as any).mock
				.calls.length;
			...
			mockInGameCommunicationChannel._simulateMessage(oldMessage);
			const callsAfterOld = (mockInGameCommunicationChannel.send as any).mock
				.calls.length;

			// Should have sent ping-pong response to correct the version
			expect(callsAfterOld).toBe(callsAfterFirst + 1);
		});

		it("should send local player position updates", () => {
			// Mock the player control to simulate movement
			mockPlayerControl.direction = { x: 1, y: 0 }; // right direction vector
	...
	describe("Guest StateSync", () => {
		...
		it("should handle host state updates and respond", () => {
			...
			mockInGameCommunicationChannel._simulateMessage(hostMessage);
		...
		it("should not respond to guest messages when guest", () => {
			...
			const sendCallsBefore = (mockInGameCommunicationChannel.send as any).mock
				.calls.length;
			...
			const sendCallsAfter = (mockInGameCommunicationChannel.send as any).mock
				.calls.length;
			expect(sendCallsAfter).toBe(sendCallsBefore); // No additional calls for guest messages
		});
	});
	...
	describe("Position conflict resolution", () => {
		...
		it("should resolve position conflicts by using latest timestamp", () => {
			...
			const initialRemotePlayer = currentState.value.players.find(
				(p) => p.id === REMOTE_PLAYER_ID,
			);
			expect(initialRemotePlayer?.position).toEqual({ x: 200, y: 200 });
			...
			mockInGameCommunicationChannel._simulateMessage(rightMoveMessage);
			...
			mockInGameCommunicationChannel._simulateMessage(downMoveMessage);
			...
			expect(updatedRemotePlayer?.position.y).toBeGreaterThan(200); // Should have moved down
			expect(updatedRemotePlayer?.lastDirection).toEqual({ x: 0, y: 1 }); // down direction vector
		});
	});
});
