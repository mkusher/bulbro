...
export class StateUpdater {
	...
	#handleStateUpdate(
		...
		this.#lastStateVersion =
			receivedMessage.version;
		const remoteEvents =
			receivedMessage.events as GameEvent[];
		const filteredRemoteEvents =
			this.#filterEvents(
				remoteEvents,
			);
		const filteredLocalEvents =
			this.#filterLocalEvents(
				localEvents,
			);
		...
		const shots =
			[
				...prevState.shots.filter(
					isFromLocalPlayer,
				),
				...receivedState.shots.filter(
					(
						shot,
					) =>
						!isFromLocalPlayer(
		...
		const newState =
			{
				...
				round:
					receivedState.round,
				mapSize:
					receivedState.mapSize,
				objects:
					receivedState.objects,
				enemies:
					receivedState.enemies,
				shots,
				players:
					[
						localPlayer,
		...
		return events.filter(
			(
				...
				if (
					...
					return this.#filterRemoteEventsForHost(
						event,
						localPlayerId,
					);
				} else {
					return this.#filterEventsForGuest(
						event,
						localPlayerId,
					);
				}
		...
		return events.filter(
			(
				...
				if (
					...
					return this.#filterLocalEventsForHost(
						event,
						localPlayerId,
					);
				} else {
					return this.#filterLocalEventsForGuest(
						event,
						localPlayerId,
					);
				}
	...
	#filterEventsForGuest(
		...
		return this.#filterRemoteEventsForGuest(
			event,
			localPlayerId,
		);
	}
