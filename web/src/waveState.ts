...
export const nextWave = (
	...
	{ occurredAt: now, deltaTime }: Extract<GameEvent, { type: "tick" }>,
): WaveState => ({
	...currentState,
	shots: [],
	objects: [],
	...
	players: currentState.players.map((player, i) =>
		player.applyEvents(
			player
				...
				.map((e) => withEventMeta(e, deltaTime, now)),
		),
	),
	round: {
		...currentState.round,
...
export const createInitialState = (
	...
	return {
		...
		players: currentPlayers.map((currentPlayer, i) =>
			spawnBulbro(
				currentPlayer.id,
				currentPlayer.bulbro.style.faceType,
	...
	const newEnemies =
		shot.shooterType === "enemy"
			? state.enemies.map((e) => {
					...
					return e.applyEvent(withEventMeta(action, deltaTime, now));
				})
			: state.enemies;
	return {
		...state,
...
export function selectWeapons(state: WaveState, action: SelectWeaponAction) {
...
export function handleBulbroInitializedForWave(
	state: WaveState,
	action: Extract<GameEvent, { type: "bulbroInitializedForWave" }>,
): WaveState {
	const { bulbroId } = action;
	return {
		...state,
...
export function handleEnemyRagingStarted(
	state: WaveState,
	action: Extract<GameEvent, { type: "enemyRagingStarted" }>,
): WaveState {
	const { enemyId } = action;
	return {
		...state,
 ...
 * Pure reducer: returns new state after applying an action.
 */
export function updateState(state: WaveState, action: Action): WaveState {
	switch (action.type) {
		case "bulbroMoved": {
		...
		case "bulbroInitializedForWave": {
			return handleBulbroInitializedForWave(state, action);
		}
		case "enemyAttacked": {
		...
		case "enemyRagingStarted": {
			return handleEnemyRagingStarted(state, action);
		}
		case "materialMoved": {
...
export const waveState = signal(
	createInitialState([], { width: 6000, height: 4500 }, 0),
);

export function fromJSON(state: WaveState) {
	waveState.value = {
		...state,
