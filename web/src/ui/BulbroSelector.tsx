...
export function BulbroSelector({
	...
	const displayedBulbro = selectedBulbro;
				...
				<div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-3 xl:grid-cols-4 gap-2">
					{bulbros.map((bulbro) => (
						<BulbroThumbnailIcon
							key={bulbro.id}
							bulbro={bulbro}
							isSelected={bulbro.id === selectedBulbro.id}
							onSelect={() => onChange(bulbro)}
						/>
					))}
				</div>
			</div>
				...
				<div className="flex flex-col md:flex-row gap-4">
					...
					<div className="flex-shrink-0">
						<WithContainerWidth>
							{({ width, height }) => (
								<BulbroCard
									bulbro={displayedBulbro}
									showDetails={false}
									displayWidth={Math.min(width, 300)}
									displayHeight={Math.max(height, 200)}
					...
					<div className="flex-1 space-y-4">
						<BulbroStats bulbro={displayedBulbro} />
						...
						<div>
							...
							<div className="flex flex-wrap gap-2">
								{displayedBulbro.defaultWeapons.map((weapon) => (
									<span
										key={weapon.id}
						...
						{/* Available Weapons */}
						{displayedBulbro.availableWeapons.length >
							displayedBulbro.defaultWeapons.length && (
							<div>
								<h4 className="font-medium text-sm text-gray-700 dark:text-gray-300 mb-2">
									Available Weapons
								</h4>
