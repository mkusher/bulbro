...
export const InteractiveSetup = {
	render: (args: any) => {
		const [character, setCharacter] = useState<CharacterSetup>({
			bulbro: bulbros.find((b) => b.id === args.bulbroId) || wellRoundedBulbro,
			sprite: (bulbros.find((b) => b.id === args.bulbroId) || wellRoundedBulbro)
				.style.faceType,
		});
		const [difficulty, setDifficulty] = useState<Difficulty>(args.difficulty);
		const [weapons, setWeapons] = useState<Weapon[]>(args.weapons);
		const [duration, setDuration] = useState<number>(args.duration);
				...
				<form onSubmit={onSubmit}>
					<Card className="flex flex-col gap-6">
						...
						<CardContent className="flex flex-col gap-6">
							<BulbroSelector
								selectedBulbro={character.bulbro}
								onChange={(bulbro) =>
									setCharacter({ bulbro, sprite: bulbro.style.faceType })
								}
							/>
	...
	argTypes: {
		...
		weapons: {
			options: [
				[pistol, fist],
				[smg, fist],
				[doubleBarrelShotgun, fist],
				[ak47, fist],
				[laserGun, fist],
				[pistol, smg],
				[ak47, doubleBarrelShotgun, fist],
			],
			control: { type: "select" },
		},
		duration: {
...
export const CharacterSelection = {
	render: (args: any) => {
		...
		const [weapons, setWeapons] = useState<Weapon[]>([pistol, fist]);
		...
		return (
			...
			>
				...
				<div style={{ flex: 1, minWidth: "300px" }}>
					...
					<BulbroSelectorView
						selectedBulbro={character.bulbro}
						selectedBulbroStyle={character.bulbro.style.faceType}
			...
			<div style={{ padding: "20px", maxWidth: "600px" }}>
				<Card>
					...
					<CardContent className="flex flex-col gap-6">
						...
						<div>
							<Label htmlFor="wave-duration">Wave duration (seconds):</Label>
							<Input
								type="number"
								name="wave-duration"
								value={duration}
						...
						>
							...
							<p>Difficulty: {difficulty} / 4</p>
							<p>Wave Duration: {duration} seconds</p>
						</div>
					</CardContent>
				</Card>
			</div>
