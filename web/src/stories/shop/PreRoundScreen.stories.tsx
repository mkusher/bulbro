...
export const CompletePreRoundScreen = {
	render: () => {
		...
		const [stats] = useState<WaveStats>({
			wave: 1,
			enemiesKilled: 45,
			damageDealt: 12450,
			damageTaken: 320,
		...
		const handlePurchase = (item: ShopItem) => {
			if (materials >= item.price && weapons.length < 6) {
				setMaterials(materials - item.price);
				setWeapons([...weapons, item.weapon]);
			}
		};

		const handleWeaponClick = (weapon: Weapon, index: number) => {
			console.log("Weapon clicked:", weapon.name, "at index", index);
		};
				...
				<div className="flex flex-col gap-3">
					...
					<div className="flex justify-center">
						<Button
							...
							onClick={() => console.log("Starting wave...")}
						>
...
export const EarlyGame = {
	render: () => {
		const [weapons, setWeapons] = useState<Weapon[]>([pistol]);
		const [materials, setMaterials] = useState(50);
		const [shopItems] = useState<ShopItem[]>([
			{ weapon: knife, price: 25 },
			{ weapon: fist, price: 10 },
			{ weapon: sword, price: 75 },
			{ weapon: smg, price: 100 },
		]);
		const [stats] = useState<WaveStats>({
			...
			<div className="p-4 max-w-6xl mx-auto">
				<div className="flex flex-col gap-3">
					...
					<div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-3">
						...
						<WeaponSlots weapons={weapons} maxSlots={6} />
					</div>
					<Shop
						items={shopItems}
						availableMaterials={materials}
